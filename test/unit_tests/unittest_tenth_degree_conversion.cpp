...
TEST(TenthDegreeConversionTests, testInTenthDegree)
{
  ...
  EXPECT_EQ(expected_tenth_degree, psen_scan::radToTenthDegree(angle_in_rad));
}
...
TEST(TenthDegreeConversionTests, testOutOfRangeAngle)
{
  ...
  EXPECT_THROW(psen_scan::radToTenthDegree(max_exceeding_angle_in_rad), std::invalid_argument);

  const double min_exceeding_angle_in_rad{ static_cast<double>(std::numeric_limits<uint16_t>::min()) - .1 };
  EXPECT_THROW(psen_scan::radToTenthDegree(min_exceeding_angle_in_rad), std::invalid_argument);
}
